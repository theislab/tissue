#### Apdated from gpu-spatial-graph-pipeline repository ####
...
def add_design_matrix(adata: Any, xl_name: str, xc_name: str, output_name: str) -> None:
    ...
    adata.obsm[output_name] = design_matrix(
        ...
        pd.get_dummies(get_from_address(adata, xl_name), dtype=float).to_numpy(),
        pd.get_dummies(get_from_address(adata, xc_name), dtype=float).to_numpy(),
    )
...
def change_type_graph_covariates(adata, key, dtype='float64'):
    """
    ...
    adata.uns["graph_covariates"]["label_tensors"][key] = adata.uns["graph_covariates"]["label_tensors"][key].astype(dtype)
